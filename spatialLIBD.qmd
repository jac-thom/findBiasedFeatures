...
*Figure 1. Bionomial deviance model better corresponds to nnSVG results*
...
*Figure 2. Binomial deviance model better corresponds to known DLPFC layer markers*
...
*Figure 3. SVGs are consistently highly ranked in binomial deviance model with and without batch*
...
*Figure 4. Influence of batch on highly deviant genes (n= 3000)*
...
top.df$is_biased = top.df$d.diff>1 | top.df$r.diff>500
...
```{r check libd genes}
#| output: false
check.genes.names = c("MTRNR2L1","MTRNR2L8","RPS4Y1","RPS26","MT-ND3","MT3","MT-CO2","MT-CO1")
check.genes = rownames(spe)[rowData(spe)$gene_name %in% check.genes.names]
names(check.genes) = rowData(spe)[check.genes,"gene_name"]
plot.df = dotplotDF(check.genes, spe, is_libd=TRUE, norm.to.mbp = F)
```

```{r plot dotplot}
dp <- ggplot(plot.df, 
       aes(x=brain, y=factor(ylabel, levels=c("MT-ND3","MT-CO2","MT-CO1",
                                              "MT3","RPS26",
                                              "RPS4Y1","MTRNR2L8","MTRNR2L1")), 
           size=n, color=scaled_avg))+
  geom_count()+
...
*Figure 5. Mitochondrial genome and potentially subject-biased genes*

```{r view mt dotplot}
#| fig-height: 3.5
#| fig-width: 7
grid.arrange(print(mt), dp, layout_matrix=matrix(c(1,1,2,2,2),, nrow=1))
```
...
*Figure 6. Influence of batch on SVGs (n= 2098)*
...
## Spatial expression plots confirm bias
...
rpsy <- plotSpots(spe, annotate=check.genes['RPS4Y1'], assay="logcounts", 
          sample_id="sample_id", point_size=.1)+ggtitle("RPS4Y1")+
  scale_color_gradient(low='grey90', high='black')
```

*Figure 7. Sex-linked genes exhibit strong subject bias*
...
The two remaining highly deviant SVGs with potential subject bias, MTRNR2L1 and MTRNR2L8, are lncRNAs with an unknown biological function. These features likewise display strong subject-biased expression and are also clearly enriched in certain spatial domains. MTRNR2L1 and MTRNR2L8 are perfect examples of features that may introduce noise if included in spatial domain clustering.
...
*Figure 8. Successful identification subject-biased, spatially variable genes*
...
```{r additional plots}
#| output: false
rps26 <- plotSpots(spe, annotate=check.genes['RPS26'], assay="logcounts", 
          sample_id="sample_id", point_size=.1)+ggtitle("RPS26")+
  scale_color_gradient(low='grey90', high='black')
mt3 <- plotSpots(spe, annotate=check.genes['MT3'], assay="logcounts", 
...
*Figure 9. RPS26 and MT3 should not be excluded due to bias*
